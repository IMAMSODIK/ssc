...
$('#store').click(function (e) {
    ...
    let potongan = ""; 
    if($("#jenis_potongan").val() == 'Persen'){
        potongan = $("#potongan").val().replace(/[^\d.]/g, '')
    }else{
        potongan = cleanRupiah($("#potongan").val()) / 100;
    }
    let min_pembelian = cleanRupiah($("#min_pembelian").val()) / 100;
    let max_potongan = cleanRupiah($("#max_potongan").val()) / 100;
    $.ajax({
        url: '/promo/store',
        method: 'POST',
        data: {
...
$("#delete-confirmed").on("click", function () {
    if(id_ctr == 0){
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    alertModal(true, "Berhasil deaktivasi data");
                    setTimeout(() => {
                        location.reload();
                    }, 2000);
                } else {
...
$("#potongan").on("input", function(){
    let jenisPromo = $("#jenis_potongan").val();
    let element = this;

    if(jenisPromo === 'Persen'){
        formatPersen(element);
    } else {
        formatRupiah(element);
    }
});

$("#jenis_potongan").on("change", function(){
    $("#potongan").val("");
});
...
function hitungDurasi() {
    ...
    if (mulai && akhir) {
        ...
        if (endDate >= startDate) {
            let durasi = Math.ceil((endDate - startDate) / (1000 * 60 * 60 * 24));
            durasiField.val(durasi + " Hari");
        } else {
            durasiField.val("Tanggal tidak valid!");
        }
    } else {
        durasiField.val("");
    }
...
$("#tanggal_mulai, #tanggal_akhir").on("change", hitungDurasi);